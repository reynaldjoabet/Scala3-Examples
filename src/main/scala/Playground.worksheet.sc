...
case class ISB(i: Double, s: String, b: Boolean, p: PrintedBook)
    derives Show,
      JsonEncoder
...
summon[Show[AudioBook]].show(aBook)
("hello", "dude").toList.mkString(",")
    ...
    new JsonEncoder[A] {
      def encode(a: A): String = {
        ...
        val fields = (fieldNames zip values).map { case (fieldName, value) =>
          s""""${fieldName}": ${
              if (value.isInstanceOf[AnyRef]) value else value
            }"""
        }
        s"{${fields.mkString(", ")}}"
      }
